...
2. מוצג `GroupPicker` לבחירת קבוצה (ה-`<select>` נשלח כ-`group_id`)
3. ה-Server Action קורא את חברי הקבוצה **בשרת** (`getGroupRosterServiceRole` ב-`src/lib/groups/roster.ts`)
   ומאכלס מהם את `lesson_students` — רשימת תלמידים מהדפדפן לא נלקחת בחשבון
4. השיעור נשמר עם `lesson_type = 'group'` ו-`lessons.group_id`

### מה נשמר (עודכן 02.09.2026)

השיעור **שומר** את מזהה הקבוצה ב-`lessons.group_id` (וסדרה חוזרת ב-`lesson_series.group_id`).
היומן, רשימת "היום" ועמוד השיעור מציגים את **שם הקבוצה** דרך `getLessonTitle()`
(`src/lib/lessons/title.ts`). המפתח הזר הוא `ON DELETE SET NULL`: מחיקת קבוצה לא מוחקת שיעורים,
והם חוזרים להיות "קבוצתי · N תלמידים". שינוי שם הקבוצה משתקף מיד בכל השיעורים.

שיעורים קבוצתיים שנוצרו לפני המיגרציה `20260902200000` שויכו אוטומטית לקבוצה שהרכבה זהה
בדיוק להרכב השיעור — רק כשיש קבוצה אחת כזו. השאר נשארו ללא `group_id`.

ה-roster של השיעור הוא **צילום** של הקבוצה ברגע היצירה. הוספת תלמיד לקבוצה לא משנה שיעורים
קיימים; הארכת סדרה (`extendLessonSeries`) מעתיקה את ה-roster מהשיעור האחרון, לא מהקבוצה.
