...
| M2 | Measurement. **Attribution capture shipped early with M1** (`ls_vid` / `ls_attr` cookies in `proxy.ts`, `attribution_touches`, frozen onto the org at signup). `tracking_destinations` + `/admin/tracking`, `<TrackingScripts />` + consent banner, Meta CAPI + GA4 Measurement Protocol and the conversion events are built (no destination is configured in production yet). **First-party landing measurement (decision #49)**: `landing_pageviews` + `LandingTracker` + `/api/telemetry/pageview`, short links `/go/<slug>` (`marketing_links`), `/admin/attribution`, Microsoft Clarity as a consent-gated, landing-only destination. Open: a real `Lead` call site, the `tracking_events` retry cron | ✅ Built, unconfigured |
