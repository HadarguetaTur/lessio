 ...
 * WhatsApp connection, but the Meta test number has no OAuth flow — its token
 * is generated by hand. Use a System User token (Business settings → System
 * users → Generate token, expiration "Never"); the API Setup token is a USER
 * token that dies after ~23h and leaves the bot silently unable to reply.
 * This script writes that connection directly onto the demo org, for the App
 * Review screencast.
 *
 * Usage (from the repo root; re-run only if the token is replaced):
...
async function main(): Promise<void> {
  ...
  console.log(
    '\nDone. Next steps:\n' +
      ...
      '  2. Open the 24h window: send "hi" from the verified number to the test number.\n' +
      '     (NOT "היי" — a Hebrew greeting persists parents.preferred_locale = he on the\n' +
      '     demo parent and every later reminder in the English screencast comes out Hebrew.)\n' +
      '  3. Re-run this script only if the stored token is replaced (it is a System User\n' +
