  ...
  return (
    <div className="flex flex-col flex-1 pb-16">
      <header className="px-4 py-3 border-b border-gray-200">
        <h1 className="font-bold text-gray-900">תשלומים</h1>
      </header>

      <main className="flex-1 p-4 space-y-6">
        {/* Pending charges */}
        <section>
          <h2 className="text-sm font-semibold text-gray-500 mb-3">ממתין לתשלום</h2>
          {pending.length === 0 ? (
            <p className="text-sm text-gray-400">אין חיובים פתוחים</p>
          ) : (
                ...
                <div key={c.id} className="bg-white border border-gray-200 rounded-lg p-3 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {chargeTypeLabel[c.charge_type] ?? c.charge_type} · {formatDate(c.created_at)}
                    </p>
                  </div>
                  {c.payment_link ? (
                    <a
        ...
        {paid.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-gray-500 mb-3">היסטוריה</h2>
            <div className="space-y-2">
              {paid.map((c) => (
