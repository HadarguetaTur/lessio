...
export function ChargeSelectionProvider({
  ...
  // that says "3 selected". Adjusted during render rather than in an effect —
  // React's documented way to reset state when a prop changes, and it avoids
  // rendering one frame with a stale selection.
  const idsKey = selectableIds.join(',')
  const [renderedKey, setRenderedKey] = useState(idsKey)
  if (renderedKey !== idsKey) {
    setRenderedKey(idsKey)
    if (selection.size > 0) setSelection(retainOnly(selection, selectableIds))
  }
