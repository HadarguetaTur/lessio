 ...
 *
 * `portalPayments` is the org's portal toggle: with the payments page closed
 * to parents there is no list of per-charge links to send them to, so several
 * open charges fall back to "talk to the teacher" rather than pointing at a
 * page that redirects to the portal home.
 */
export function resolvePaymentLine(
  charges: OpenCharge[],
  locale: AppLocale,
  portalPayments = true
): string {
  ...
  if (withLink.length > 0 && portalPayments) {
